<script lang="ts">
  ...
  import { exportBackup, saveBackupFile } from "$lib/stores/accounts";
  ...
  async function handleExport() {
    ...
    try {
      ...
      if (isMobile()) {
        // Mobile: save via Rust command (share sheet on iOS, Downloads on Android)
        await saveBackupFile(bytes);
